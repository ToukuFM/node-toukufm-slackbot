...
var strings_faq = require('./strings/faq.json');
...
var __animeHelpString = function() {
...
var __parseAnimeFeed = function(header, results) {
        ...
        if (term == 'latest') {
            Nyaa.get_latest(function(err, results) {
                if (err) {
                    ...
                    callback(__parseAnimeFeed(header, results));
                }
            });
        }
...
var _prepareFaq = function(data, ctx, callback) {
    var response = '';
    for (var string in strings_faq)
        response += strings_faq[string] + '\n';
    callback(response);
};
...
var commands = {
    ...
    'pomf': (data, ctx, callback) => callback("_Wah! What are we gonna do on the bed?_")
};
...
var slack = (function() {
    ...
    // Set your Bot user ID, you can find this by looking at the log and
    // mentioning your @botname, it'll be transformed into a <@token>, 
    // and that is the token you'll need as SLACK_BOTNAME
    // $ export SLACK_BOTNAME=U0J1BG81G
    var botname = '<@' + process.env.SLACK_BOTNAME + '>';
    ...
    var _onMessage = function(data) {
        ...
        // Received a command
        var isValidMention = data.text.startsWith(botname);
        if (data.text.charAt(0) === prefix || isValidMention) {
            
            // Split the command and it's arguments into an array
            var command;
            if (isValidMention) {
                command = data.text.substring(botname.length).split(' ');
                command.splice(0, 1);
